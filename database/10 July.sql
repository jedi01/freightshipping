...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 10, 2019 at 05:49 PM
...
--
-- Table structure for table `ip_additional_loads`
--

CREATE TABLE `ip_additional_loads` (
  `additional_id` int(11) NOT NULL,
  `customer` varchar(20) NOT NULL,
  `pickupdate` date NOT NULL,
  `shipper` varchar(50) NOT NULL,
  `receiver` varchar(50) NOT NULL,
  `origincarrier` varchar(50) NOT NULL,
  `pickuptime` date NOT NULL,
  `shipping_order_id` int(11) NOT NULL,
  `punotes` varchar(200) NOT NULL,
  `orderdate` date NOT NULL,
  `bol` varchar(200) NOT NULL,
  `orderstatus` varchar(50) NOT NULL,
  `trailernum` varchar(50) NOT NULL,
  `netweight` varchar(50) NOT NULL,
  `seal` varchar(50) NOT NULL,
  `piececount` varchar(50) NOT NULL,
  `trailersize` varchar(50) NOT NULL,
  `trailertype` varchar(50) NOT NULL,
  `notes` text NOT NULL,
  `instructions` text NOT NULL,
  `etadate` date NOT NULL,
  `destcarrier` varchar(100) NOT NULL,
  `deldate` date NOT NULL,
  `delappt` varchar(200) NOT NULL,
  `delnotes` text NOT NULL,
  `rate` varchar(200) NOT NULL,
  `fsc` varchar(200) NOT NULL,
  `currency` varchar(20) NOT NULL,
  `railcompleted` varchar(200) NOT NULL,
  `truckcompleted` varchar(200) NOT NULL,
  `delivered` varchar(200) NOT NULL,
  `customscleared` varchar(200) NOT NULL,
  `invoicesubmitted` varchar(200) NOT NULL,
  `Brokerid` varchar(20) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Dumping data for table `ip_additional_loads`
--

INSERT INTO `ip_additional_loads` (`additional_id`, `customer`, `pickupdate`, `shipper`, `receiver`, `origincarrier`, `pickuptime`, `shipping_order_id`, `punotes`, `orderdate`, `bol`, `orderstatus`, `trailernum`, `netweight`, `seal`, `piececount`, `trailersize`, `trailertype`, `notes`, `instructions`, `etadate`, `destcarrier`, `deldate`, `delappt`, `delnotes`, `rate`, `fsc`, `currency`, `railcompleted`, `truckcompleted`, `delivered`, `customscleared`, `invoicesubmitted`, `Brokerid`) VALUES
(9, '3', '2019-06-20', '1', '1', '2', '2019-06-28', 38903, 'sdaf', '2019-06-20', 'asdf', '1', 'asdf', 'asdf', 'asdf', 'asdf', '1', '', 'asdf', 'asdf', '2019-06-13', '3', '2019-06-18', 'asdf', 'asdf', 'asdf', 'asdf', 'USD', 'asdf', 'asdf', 'asdf', 'asdf', 'asdf', '1'),
(10, '3', '2019-06-04', '2', '2', '2', '2019-06-06', 38916, 'sadf', '2019-06-19', 'asdf', '4', 'asdf', 'asdf', 'asdf', 'asdf', '2', '', 'asdf', 'asdf', '2019-06-05', '2', '2019-06-11', 'asdf', 'asd', 'asdf', 'asdf', 'USD', 'df', 'asdfas', 'asdf', 'asdf', 'asdf', '2');
...
(25, '2019-07-08 19:59:13', '2019-07-08 19:59:13', 'test', 'test', 'test', 'sdf', 'sdaf', 'sdfs', 'AS', '2342', '234', NULL, 'sdfds', NULL, NULL, NULL, 'system', 1, NULL, '23423', 'sdfds', NULL, NULL, NULL, NULL, 0);
...
(35013, 38901, '2019-06-04', 'dfsg', '1', 'gdfgsdf');
...
--
-- Stand-in structure for view `movestrack`
-- (See below for the actual view)
--
CREATE TABLE `movestrack` (
`movesid` int(11)
...
--
-- Stand-in structure for view `shippingoders`
-- (See below for the actual view)
--
CREATE TABLE `shippingoders` (
`shippingorderid` int(11)
,`client_name` text
,`pickupdate` date
,`shippername` varchar(50)
...
--
-- Structure for view `movestrack`
--
DROP TABLE IF EXISTS `movestrack`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `movestrack`  AS  select `ip_moves`.`movesid` AS `movesid`,`ip_moves`.`shippingorderid` AS `shippingorderid`,`ip_carriers`.`carriername` AS `carriername`,`ip_shippers`.`shippername` AS `shippername`,`ip_receivers`.`receivername` AS `receivername`,`ip_moves`.`pickupdate` AS `pickupdate`,`ip_moves`.`deldate` AS `deldate` from (((`ip_moves` join `ip_shippers` on((`ip_shippers`.`shipperid` = `ip_moves`.`shipper`))) join `ip_receivers` on((`ip_receivers`.`receiverid` = `ip_moves`.`receiver`))) join `ip_carriers` on((`ip_carriers`.`carrierid` = `ip_moves`.`carrier`))) ;

-- --------------------------------------------------------

--
-- Structure for view `shippingoders`
--
DROP TABLE IF EXISTS `shippingoders`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `shippingoders`  AS  select `ip_shipping`.`shippingorderid` AS `shippingorderid`,`ip_clients`.`client_name` AS `client_name`,`ip_shipping`.`pickupdate` AS `pickupdate`,`ip_shippers`.`shippername` AS `shippername`,`ip_receivers`.`receivername` AS `receivername`,`ip_carriers`.`carriername` AS `carriername`,`ip_shipping`.`customernumber` AS `customernumber`,`ip_shipping`.`orderdate` AS `orderdate`,`ip_shipping`.`bol` AS `bol`,`ip_shipping`.`orderstatus` AS `orderstatus`,`ip_shipping`.`etadate` AS `etadate`,`ip_dispatcher`.`name` AS `name`,`ip_shipping`.`id` AS `id` from ((((((((`ip_shipping` join `ip_clients` on((`ip_clients`.`client_id` = `ip_shipping`.`customer`))) join `ip_shippers` on((`ip_shippers`.`shipperid` = `ip_shipping`.`shipper`))) join `ip_receivers` on((`ip_receivers`.`receiverid` = `ip_shipping`.`receiver`))) join `ip_carriers` on((`ip_carriers`.`carrierid` = `ip_shipping`.`origincarrier`))) join `ip_trailersize` on((`ip_trailersize`.`trailersizeid` = `ip_shipping`.`trailersize`))) join `ip_trailertype` on((`ip_trailertype`.`trailertypeid` = `ip_shipping`.`trailertype`))) join `ip_custombrokers` on((`ip_custombrokers`.`brokerid` = `ip_shipping`.`brokerid`))) join `ip_dispatcher` on((`ip_dispatcher`.`dispatcherid` = `ip_shipping`.`dispatcher`))) ;
...
--
-- Indexes for table `ip_additional_loads`
--
ALTER TABLE `ip_additional_loads`
...
--
-- AUTO_INCREMENT for table `ip_additional_loads`
--
ALTER TABLE `ip_additional_loads`
  MODIFY `additional_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
...
--
ALTER TABLE `ip_trackingdetail`
  MODIFY `trackingdetailid` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=35014;
